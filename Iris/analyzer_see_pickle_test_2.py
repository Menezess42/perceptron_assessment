...
console = Console() # Instancia com console do Rich para imprimir na tela com formatação.
...
def print_table(data, max_rows=20):
    ...
    for column in data.columns:
        # Adiciona cada coluna como um cabeçalho destacados.
        table.add_column(column)

    # Define quais linhas mostrar (limitados por max_rows, se especificado)
    rows_to_show = data if max_rows is None else data.head(max_rows)

    for _, row in rows_to_show.iterrows():
        # Para cada linha do dataFrame, adiciona uma linha na tabela Rich
        table.add_row(*[str(row[col]) for col in data.columns])

    console.print(table) # Imprime a tabela formato no console.
...
def main():
    # Função principal que roda o sistema de visualização.
    ...
    while True:
        ...
        if command == "exit":
            ...
            try:
                if ":" in col_input:
                    # Se for invervalo (ex: a:c), converte para índices e fatiamento
            ...
            try:
                if row_input:
                    ...
                    # Se não tiver slice, filtra só por coluna
