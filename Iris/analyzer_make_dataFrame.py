...
class Analyzer_test_1:
    ...
    def __init__(self, base_dir="./Reports/test_1/"):
        """
        Carraga o caminho onde os relatórios .json estão.
        """
    ...
    def load_all(self):
        ...
        for weight_dir in sorted(self.base_dir.glob("weights_*")): 
            ...
            for run_dir in sorted(weight_dir.glob("run_*")):
                ...
                record = {
                    "weight_init": weight_key, # Salva a chave de inicialização dos pesos
                    "run": run_index, # Slava o número da execução
                }

                best_model_file = next(run_dir.glob("best_model_epoch_*.json"), None)
                # Procura o arquvio JSON com o melhor modelo salvo (com nome tipo: best_model_epoch_5.json)
                ...
                if validation_file.exists():
                    # Vericia se o arquivo de validação existe
        ...
        for lr_dir in sorted(self.base_dir.glob("lr_*")):
            ...
            for run_dir in sorted(lr_dir.glob("run_*")):
                ...
                record = {
                    ...
                    "run": run_index, # Salva o número de execução
                }

                best_model_file = next(run_dir.glob("best_model_epoch_*.json"), None)
                # Procura o arquivo JSON com o melhor modelo salvo (com nome tipo: best_model_epoch_5.json)
                # Se não encontrar, best_model_file fica como None
                if best_model_file:
...
if __name__ == "__main__":
    ...
    analyzer_t1.save_dataframe(path='./Reports/reports_test_1/compiled_data.pkl')
    analyzer_t2 = Analyzer_test_2()
    analyzer_t2.load_all()
    analyzer_t2.save_dataframe(path='./Reports/reports_test_1/compiled_data_test_2.pkl')
